...
printf("If network input from kybd give(l)(file=0);unknw input kybd 1;& output to m
onitor 1 (file=0)\r\n");
...
do{
if(in5) p rin tf(”%4d Unknowns must be given.Give no. of each of following\r\n",nunk)
7
for(i=0;i<6;i++){
if(in 5 ){
p rin tfp 1 %ld . %s 11, i + 1, cuk [i] ) ;
scanf("%d",&muk[i] ) ;
}
...
i f (11 [i]<0){xx=x[abs(11[i])-1+nvar] ;
ii = 0 ;
} else {xx=x[11[i]-1];ii = ll[i] ;
}
i f ((xx-x[12[i]-1])*x[i-l+nj2]> -l.e-5) goto L28;
...
printf("\r\n");
printf("Pipe Node Node
Length Diameter Roughness Flowrate Headloss\r\n") ;
printf(" No.
#1
#2
Coef.\r\n");
